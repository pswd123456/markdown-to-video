...
class rag_06_conclusion(Scene):
    def construct(self):
        ...
        silhouette.set_color(WHITE).scale(1.2)
        silhouette.move_to(ORIGIN)

        # Fallback if SVG not available: use a simple circle with label
        if silhouette.get_num_points() == 0:
            silhouette = Circle(radius=0.8, color=WHITE, fill_opacity=0)
            dev_label = Text("开发者", font="Noto Sans CJK SC", font_size=36, color=WHITE)
            dev_label.move_to(silhouette.get_center())
            silhouette = VGroup(silhouette, dev_label)
        ...
        # Animate nodes and arrows in sequence
        ...
        # Create a large green checkmark for background behind silhouette
        big_check = Text("✓", font="Noto Sans CJK SC", font_size=200, color=GREEN)
        big_check.move_to(silhouette.get_center()).set_opacity(0.15)

        # Fade out all elements except title, and overlay subtle checkmark behind center
